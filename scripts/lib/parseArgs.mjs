 ...
 *  with a line per request — `--fresh` (dev-up.mjs only) to bypass the seed cache (see
 *  scripts/lib/seedCache.mjs) and force a full reseed even on a cache hit — and `--loggedout`
 *  (dev-up.mjs only) to skip the default auto-login and print a plain dashboard URL that lands on
 *  the login screen instead. */
export function parseArgs(argv) {
  const args = { id: "default", verbose: false, fresh: false, loggedout: false };
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (argv[i] === "--loggedout") args.loggedout = true;
  }
