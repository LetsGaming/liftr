...
| `pnpm i18n:check` | `node scripts/i18n-check.mjs` — verifies `en.yaml` has exactly the same keys as `de.yaml` (the source of truth) with matching `{placeholder}` names, plus the same check for the generated `exercises.{de,en}.json` pair. Run this after touching any UI copy or locale file; see [`docs/guides/adding-a-language.md`](../guides/adding-a-language.md). |
