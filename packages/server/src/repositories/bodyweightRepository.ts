...
export function findLatestBodyweightLog(db: LiftrDb, userId: string) {
  return db.query.bodyweightLogs.findFirst({ where: eq(bodyweightLogs.userId, userId), orderBy: desc(bodyweightLogs.date) });
}
...
export async function upsertBodyweightLog(db: LiftrDb, userId: string, date: string, weightKg: number) {
  ...
  const [row] = await db.insert(bodyweightLogs).values({ userId, date, weightKg }).returning();
  return row!;
}
