...
export async function readJsonSetting<T>(db: LiftrDb, userId: string, key: string): Promise<T | null> {
  const row = await db.query.settings.findFirst({ where: and(eq(settings.userId, userId), eq(settings.key, key)) });
  return row ? (JSON.parse(row.value) as T) : null;
}

export async function writeJsonSetting(db: LiftrDb, userId: string, key: string, value: unknown): Promise<void> {
  await db
    .insert(settings)
    .values({ userId, key, value: JSON.stringify(value) })
    .onConflictDoUpdate({ target: [settings.userId, settings.key], set: { value: JSON.stringify(value) } });
}
