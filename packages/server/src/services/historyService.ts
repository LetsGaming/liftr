...
/** Unified reverse-chronological feed of finished workouts + runs. */
 ...
 */
export async function getHistoryPage(db: LiftrDb, userId: string, cursor: string | undefined, limit: number): Promise<HistoryPage> {
  ...
  const [finishedWorkouts, recentRuns] = await Promise.all([
    findFinishedWorkoutsPage(db, userId, before, limit),
  ...
  const rankRows = await findRanksByExerciseIds(db, userId, usedExerciseIds);
  ...
  const items: HistoryItem[] = [
    ...finishedWorkouts.map((w): HistoryItem => {
      ...
      return {
        ...
        id: w.id,
        at: w.startedAt,
        // `routine` can be null (Quick Start has no routineId).
