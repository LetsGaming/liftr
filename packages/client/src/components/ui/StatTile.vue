 ...
 *  giving no visual cue that "1 Tage Serie" and "LEHRLING I" meant different things. `accent`
 *  gives a tile its own fixed color instead, reusing the conventions already established for
 *  these two axes elsewhere (App.vue's .streak-chip text is --fire-hi; .level-chip's xp-amount
 *  and the XP/level .rankbar's documented fallback are --blue-hi) rather than inventing new
 *  colors. */
// `label` stays a plain string for the common case; a caller needing an icon in front of the
// label text (e.g. the streak tile's flame icon) can use the `#label` slot instead, which
// overrides the prop.
