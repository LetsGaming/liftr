...
import { workoutExercises, workouts, type LiftrDb } from "@liftr/db";
...
describe("insertSet", () => {
  it("creates and returns the new set row", async () => {
    const row = await insertSet(db, newSet({ clientId: "s1", weightKg: 100, reps: 5 }));
  ...
  it("allows a null weightKg for bodyweight sets", async () => {
    const row = await insertSet(db, newSet({ clientId: "s-bw", weightKg: null }));
...
describe("findSetByClientId", () => {
  it("returns the set matching the given clientId", async () => {
    await insertSet(db, newSet({ clientId: "s-find-me" }));

    const result = await findSetByClientId(db, "s-find-me");

    expect(result?.clientId).toBe("s-find-me");
  });

  it("returns undefined when no set has that clientId", async () => {
    const result = await findSetByClientId(db, "nonexistent-client-id");
