...
describe("findAllRanks / findAllRanksWithExercise", () => {
  it("return an empty array when nothing has been computed", async () => {
    expect(await findAllRanks(db)).toEqual([]);
    expect(await findAllRanksWithExercise(db)).toEqual([]);
  });

  it("findAllRanksWithExercise nests the related exercise row", async () => {
    const ex = await insertTestExercise(db, { slug: "rr-ex-2" });
    await upsertRank(db, baseRankUpsert(ex.id));

    const result = await findAllRanksWithExercise(db);
  ...
  it("findAllRanks returns the bare rank row without a joined exercise", async () => {
    ...
    const result = await findAllRanks(db);
...
describe("findLoggedSetsForExercise", () => {
  it("excludes warmup sets", async () => {
    ...
    const result = await findLoggedSetsForExercise(db, ex.id);
...
describe("upsertRank / findRankByExerciseId", () => {
  it("inserts a new rank row when none exists", async () => {
    ...
    await upsertRank(db, baseRankUpsert(ex.id, { tier: "trainee", division: 3, lp: 40 }));

    const result = await findRankByExerciseId(db, ex.id);
  ...
  it("overwrites the existing rank row on conflict rather than duplicating", async () => {
    ...
    await upsertRank(db, baseRankUpsert(ex.id, { tier: "initiate", division: 5, lp: 10 }));

    await upsertRank(db, baseRankUpsert(ex.id, { tier: "athlete", division: 2, lp: 90 }));
    ...
    const all = await findAllRanks(db);
...
describe("findBestPrByKind / insertPr", () => {
  it("returns the highest-value pr for the given kind", async () => {
    ...
    await insertPr(db, { exerciseId: ex.id, kind: "weight", value: 80, achievedAt: new Date("2026-08-01T10:00:00Z") });
    await insertPr(db, { exerciseId: ex.id, kind: "weight", value: 100, achievedAt: new Date("2026-09-01T10:00:00Z") });
    await insertPr(db, { exerciseId: ex.id, kind: "weight", value: 90, achievedAt: new Date("2026-08-15T10:00:00Z") });

    const result = await findBestPrByKind(db, ex.id, "weight");
  ...
  it("does not mix PRs of a different kind", async () => {
    ...
    await insertPr(db, { exerciseId: ex.id, kind: "weight", value: 100, achievedAt: new Date() });
    await insertPr(db, { exerciseId: ex.id, kind: "reps", value: 200, achievedAt: new Date() });
  ...
  it("returns undefined when no pr of that kind exists", async () => {
    ...
    const result = await findBestPrByKind(db, ex.id, "e1rm");
...
describe("insertRankEvent / findRankEventsSince", () => {
  it("excludes events that occurred before the cutoff", async () => {
    ...
    await insertRankEvent(db, { exerciseId: ex.id, tier: "trainee", division: 3, occurredAt: new Date("2026-08-01T10:00:00Z"), plausibilityReason: null });
    await insertRankEvent(db, { exerciseId: ex.id, tier: "athlete", division: 2, occurredAt: new Date("2026-09-05T10:00:00Z"), plausibilityReason: null });

    const result = await findRankEventsSince(db, new Date("2026-09-01T00:00:00Z"));
  ...
  it("carries the plausibility reason through", async () => {
    ...
    await insertRankEvent(db, { exerciseId: ex.id, tier: "lifter", division: 1, occurredAt: new Date("2026-09-05T10:00:00Z"), plausibilityReason: "pace" });
  ...
  it("includes events exactly at the cutoff (inclusive lower bound)", async () => {
    ...
    await insertRankEvent(db, { exerciseId: ex.id, tier: "trainee", division: 3, occurredAt: cutoff, plausibilityReason: null });

    const result = await findRankEventsSince(db, cutoff);
